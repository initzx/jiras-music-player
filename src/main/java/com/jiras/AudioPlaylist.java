...
public class AudioPlaylist extends Application {
    ...
    public void start(final Stage stage) throws Exception {
        ...
        File musicDir = new File("/home/init0/Music/ASTROWORLD");

        for (String file: musicDir.list()) {
            players.add(new MediaPlayer(new Media(Paths.get(musicDir+"/"+file).toUri().toString())));
        }

        if (players.isEmpty()) {
            System.out.println("No audio found in " + musicDir);
            Platform.exit();
            return;
        }
